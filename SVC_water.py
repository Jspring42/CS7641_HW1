...
def main():
    ...
    plt.title("Water SVC Validation Curve (C)")
    plt.xlabel("C")
    plt.ylabel("Accuracy")
    # plt.ylim(0.0, 1.1)
    lw = 2
    ...
    plt.fill_between(param_range, test_scores_mean - test_scores_std,
                     ...
                     color="navy", lw=lw)
    plt.legend(loc="best")
    fig.savefig('SVC_water_C.png')
    
    param_range = ['linear', 'poly', 'rbf', 'sigmoid']
    train_scores, test_scores = validation_curve(SVC(C=100),
                                                  X, Y, 
                                                  param_name="kernel",
                                                  param_range=param_range,
                                                  cv=5,
    ...
    plt.title("Water SVC Validation Curve (kernel)")
    plt.xlabel("kernel")
    plt.ylabel("Accuracy")
    # plt.ylim(0.0, 1.1)
    lw = 2
    ...
    plt.fill_between(param_range, test_scores_mean - test_scores_std,
                     ...
                     color="navy", lw=lw)
    plt.legend(loc="best")
    fig.savefig('SVC_water_kernel.png')    
    ...
    axes[0].set_title("Water SVC Learning Curve/Size")
    ...
    axes[1].set_title("Water SVC Learning Curve/Iteration Time")
    fig.savefig('SVC_water_lc.png')    
    ...
    plt.title ("Water SVC Confusion Matrix", fontdict = {'size':18}, pad = 20)
    plt.savefig('SVC_water_holdout_pred.png', figsize=[12,9])    
